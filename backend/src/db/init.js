...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS leads (
    ...
    owner_id TEXT NOT NULL,
    notes TEXT,
    hook_suggestions TEXT, -- JSON array stored as text
    competitor_info TEXT,
    trigger_events TEXT, -- JSON array stored as text
    created_at TEXT DEFAULT (datetime('now')),
    updated_at TEXT DEFAULT (datetime('now')),
  ...
  CREATE TABLE IF NOT EXISTS transcripts (
    ...
    insights TEXT, -- JSON stored as text (pain_points, stakeholders, red_flags, next_steps)
  ...
  CREATE TABLE IF NOT EXISTS sales_rooms (
    ...
    video_url TEXT,
    calendly_link TEXT,
    branding TEXT, -- JSON stored as text (logo_url, primary_color, secondary_color)
    is_expired INTEGER DEFAULT 0,
    expires_at TEXT,
    password_protected INTEGER DEFAULT 0,
    password_hash TEXT,
    mutual_action_plan TEXT, -- JSON stored as text
  ...
  CREATE TABLE IF NOT EXISTS battlecards (
    ...
    arc_response TEXT NOT NULL, -- JSON stored as text (acknowledge, reframe, counter)
    case_study_links TEXT, -- JSON array stored as text
    is_shared INTEGER DEFAULT 0,
...
const existingAdmin = db.prepare('SELECT id FROM users WHERE email = ?').get('admin@salesroom.local');
...
if (!existingAdmin) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(adminId, 'admin@salesroom.local', passwordHash, 'System Admin', 'admin', 1);
