...
async function executeSearch(searchId, missionObjective, icpTemplateId, userId) {
  try {
    ...
    for (const result of results) {
      ...
      await run(`
        INSERT INTO leads (id, name, company_name, industry, tech_stack, identified_pain, confidence_score, source_link, status, search_id, owner_id, hook_suggestions, competitor_info, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `, [
        leadId,
        result.name || 'Unknown Contact',
        result.company_name,
        result.industry,
